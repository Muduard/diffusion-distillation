...
import os
from old_scheduling import CustomScheduler, ScheduleTypes
@dataclass
...
def evaluate(config, epoch, model, gen, timestep_number, schedule, timestep_type):
...
pretrained_student = "c10/1epoch4"#config.output_dir + "/0epoch5/" #""##  #

model_id = "google/ddpm-cifar10-32"

# load model and scheduler
teacher = UNet2DModel.from_pretrained("c10/eps_epoch22")#DDPMPipeline.from_pretrained(model_id).unet # ##
        ...
        for epoch in range(starting_epoch, config.num_epochs):
            ...
            if accelerator.is_main_process:
                if (epoch + 1) % config.save_image_epochs == 0 or epoch == config.num_epochs - 1:
                    evaluate(config, epoch, student, gen, int(N/2), ScheduleTypes.COSINE, "continuous")
...
teacher = UNet2DModel.from_pretrained("./c10/eps_epoch22/", device_map="auto")
evaluate(config, 500, teacher, gen, 16, ScheduleTypes.COSINE)
