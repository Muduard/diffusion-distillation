...
#model = UNet2DModel.from_pretrained("c10/eps_logsnr4")
...
def train_loop(config, model, noise_scheduler, optimizer, train_dataloader, lr_scheduler):
    ...
    for epoch in range(starting_epoch, config.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            logsnr_emb = (torch.arctan(torch.exp(-0.5 * torch.clip(logsnr, -20., 20.))) / (0.5 * torch.pi))
            ...
            with accelerator.accumulate(model):
                ...
                eps_pred = model(z_t, logsnr_emb, return_dict=False)[0]
                epsT_pred = batch_last(eps_pred)
                xT_pred = eps_to_x(zT_t, logsnr, epsT_pred)
                x_pred = batch_first(xT_pred)
        ...
        if accelerator.is_main_process:
            if (epoch + 1) % config.save_image_epochs == 0 or epoch == config.num_epochs - 1:
                evaluate(config, epoch, model, gen, N, ScheduleTypes.COSINE, PredTypes.x, "continuous")

            if (epoch + 1) % config.save_model_epochs == 0 or epoch == config.num_epochs - 1:
                file_path = config.output_dir + f'/eps_logsnr{epoch}/'
...
train_loop(*args)
model = UNet2DModel.from_pretrained("c10/eps_logsnr4", local_files_only=True)
model = model.to(device)
evaluate(config, 1000, model, gen, 1024, ScheduleTypes.COSINE, PredTypes.x, "continuous")
