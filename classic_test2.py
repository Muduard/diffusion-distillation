...
'''model = UNet2DModel(
    ...
    block_out_channels=(256, 256,256, 256, 256),  # the number of output channes for each UNet block
    down_block_types=(
        "DownBlock2D",  # a regular ResNet downsampling block
        "AttnDownBlock2D",  # a ResNet downsampling block with spatial self-attention
        "DownBlock2D",
...
#model = UNet2DModel.from_pretrained("c10/x_good1")
model = model.to(device)

noise_scheduler = CustomScheduler("cuda",
                                  ScheduleTypes.COSINE)  # DDIMScheduler(num_train_timesteps=1024, beta_schedule="squaredcos_cap_v2")
...
def train_loop(config, model, noise_scheduler, optimizer, train_dataloader, lr_scheduler):
    ...
    for epoch in range(starting_epoch, config.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            i = timesteps #torch.add(timesteps, other=1)
            i = i / N
            # logsnr = compute_sigma_t(i)
            #alpha_t = alphas_cumprod[ts[timesteps]] ** 0.5
            alpha_t = compute_alpha(i)
            sigma_t = compute_sigma(alpha_t).to(device)
            # alpha_t = torch.sqrt(sigmoid(logsnr))  # alphas[ts2[i]]
            # sigma_t = compute_sigma(alpha_t).to(x.device)
            # sigma_t = torch.sqrt(sigmoid(-logsnr))
            ...
            with accelerator.accumulate(model):
                ...
                eps_pred = model(z_t, i, return_dict=False)[0]
                #v_pred = model(z_t, i, return_dict=False)[0]
                #epsT_pred = batch_last(eps_pred)
                #xT_pred = eps_to_x(zT_t, logsnr, epsT_pred)
                #xT_pred = batch_last(x_pred)
        ...
        if accelerator.is_main_process:
            ...
            if (epoch + 1) % config.save_model_epochs == 0 or epoch == config.num_epochs - 1:
                file_path = config.output_dir + f'/eps_good{epoch}/'
...
train_loop(*args)
model = UNet2DModel.from_pretrained("c10/eps_epoch22", local_files_only=True)
model = model.to(device)
evaluate(config, 1001, model, gen, 512, ScheduleTypes.COSINE, PredTypes.eps, "continuous")