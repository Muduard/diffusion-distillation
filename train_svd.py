...
def get_svd_from_batch(x):
    bs = torch.zeros((x.shape[1],x.shape[2],x.shape[0]),device=x.device)
    for i in range(x.shape[0]):
        bs[:,:,i] = get_color_svd(x[i,:,:,:])
    return bs

def get_color_svd(x):
    us = []
    ss = torch.zeros((x.shape[0], x.shape[1]),device=x.device)
    vs = []
    for i in range(x.shape[0]):
        u, s, v = torch.linalg.svd(x[i, :, :])
        ss[i,:] = s
...
def train_loop(config, model, noise_scheduler, optimizer, train_dataloader, lr_scheduler):
    ...
    for epoch in range(starting_epoch, config.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            s = get_svd_from_batch(x)
            epsilon = torch.randn(s.shape).to(x.device)
            s2 = (alpha_t * s + sigma_t * epsilon)
            ...
            with accelerator.accumulate(model):
                ...
                eps_pred = model(batch_first(s2), tsN[timesteps], return_dict=False)[0]
...
#evaluate(config, 25, model, gen, 1024, ScheduleTypes.COSINE, PredTypes.eps)
